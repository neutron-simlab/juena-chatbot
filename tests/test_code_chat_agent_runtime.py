...
async def test_code_chat_agent_reuses_compiled_resources(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_create_agent(**kwargs: Any) -> object:
        ...
        created.setdefault("create_agent_calls", []).append(kwargs)
    ...
    composite_backend = repo_backend_factory(type("Runtime", (), {"state": {}})())
...
def test_code_chat_prompts_cover_staged_user_inputs() -> None:
    assert "code-chat coordinator" in code_chat_agent.CODE_CHAT_SYSTEM_PROMPT
    assert "/inputs/" in code_chat_agent.CODE_CHAT_RESEARCH_SUBAGENT_PROMPT
    assert "`code-chat-expert`" in code_chat_agent.CODE_CHAT_SYSTEM_PROMPT
    assert "Do not answer repository or codebase questions from model memory" in code_chat_agent.CODE_CHAT_SYSTEM_PROMPT
    assert "instead of editing staged files" in code_chat_agent.CODE_CHAT_RESEARCH_SUBAGENT_PROMPT
    assert "Local indexed repositories come first" in code_chat_agent.CODE_CHAT_SYSTEM_PROMPT
    assert "Context7 comes" in code_chat_agent.CODE_CHAT_SYSTEM_PROMPT
    assert "Tavily web search is" in code_chat_agent.CODE_CHAT_SYSTEM_PROMPT
    assert "Every substantive claim in the final answer must be grounded" in code_chat_agent.CODE_CHAT_SYSTEM_PROMPT
    ...
    assert "combines FTS5 keyword retrieval with vector-semantic search" in code_chat_agent.CODE_CHAT_RESEARCH_SUBAGENT_PROMPT
